...
import { PROTECTED_ROUTES_CORE } from "./utils/constants";
...
test.describe("Smoke Test — Core Routes", () => {
  test("SMOKE-001-CORE: All core (dashboard) protected routes load without crash", async ({ page }) => {
    test.setTimeout(90_000);

    logger.step("Begin crawling core protected routes");

    const failures: { route: string; reason: string }[] = [];

    for (const route of PROTECTED_ROUTES_CORE) {
    ...
    logger.info(`Crawled ${PROTECTED_ROUTES_CORE.length} core routes. Failures: ${failures.length}`);
