...
export type CrucibleTarget = {
  schema: z.ZodTypeAny;
  generateHappy: (seeds: SeedContext, faker: Faker) => Record<string, unknown>;
  isPublicWebhook?: boolean;
  expectedHappyStatuses?: number[];
};
...
export const CrucibleRegistry: Record<string, CrucibleTarget> = {
  "smart-roster-match": {
    schema: z.object({
      blueprint_id: uuid,
  ...
  "process-integration-sync-queue": {
    schema: z.object({}),
    generateHappy: () => ({}),
  },
...
export function getCrucibleTarget(name: string): CrucibleTarget {
