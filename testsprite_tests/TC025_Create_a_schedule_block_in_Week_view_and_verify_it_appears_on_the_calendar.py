...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /auth (the test explicitly requests navigation to /auth as the next action).
        ...
        # -> Type email into the email field (index 2094), type password into the password field (index 2089), then click the 'Sign In' button (index 2096).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Retry the Sign In action by clicking the 'Sign In' button (index 2096) one more time to attempt login again.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div[6]/div/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
