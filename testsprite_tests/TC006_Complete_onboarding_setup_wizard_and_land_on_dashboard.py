...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields with the provided test credentials, then click the 'Sign In' button to attempt login.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Retry signing in by clicking the 'Sign In' button (element index 2096) to see if the login completes and the app redirects to the onboarding (/setup) flow.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div[6]/div/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
