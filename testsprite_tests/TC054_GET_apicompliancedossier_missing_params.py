...
def test_get_compliance_dossier_missing_params():
    ...
    try:
        ...
        # Expecting HTTP 200 and content-type application/pdf as per PRD
        assert response.status_code == 200, f"Expected status 200 but got {response.status_code}, response text: {response.text}"
        content_type = response.headers.get('Content-Type')
        assert content_type == 'application/pdf', f"Expected Content-Type application/pdf but got {content_type}"
