...
async def run_test():
    ...
    try:
        ...
        # -> Type the test email into the email field (index 2048) and then enter the password and submit the form.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click on 'Jobs' in the dashboard navigation to open the jobs list (element index 2196).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div/aside/nav/div/div/div/div/div/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Jobs' navigation link (index 2196) to open the jobs list so a job row can be selected.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div/aside/nav/div/div/div/div/div/a').nth(0)
        await asyncio.sleep(3); await elem.click()
