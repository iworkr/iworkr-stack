...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign in' link to open the authentication page (/auth).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/header/nav/div[2]/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the test email into the email field (index 2028) and password into the password field (index 2023), then click the 'Sign In' button (index 2030).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click 'Dispatch' in the main navigation (element index 2178) to open the Dispatch page and then verify the Dispatch URL and Live map visibility.
        frame = context.pages[-1]
        # Click element
        ...
        assert await frame.locator("xpath=//*[contains(., 'Live map')]").nth(0).is_visible(), "Expected 'Live map' to be visible"
