...
def test_post_api_webhook_polar_deprecated():
    url = f"{BASE_ENDPOINT}/webhook/polar"
    try:
        # Sending empty JSON body with POST, no cookies (no auth)
        response = requests.post(url, json={}, timeout=30)
        assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
        json_resp = response.json()
        assert isinstance(json_resp, dict), f"Response is not a JSON object: {json_resp}"
        assert json_resp.get("received") is True, f"Expected received:true, got {json_resp.get('received')}"
        assert json_resp.get("deprecated") is True, f"Expected deprecated:true, got {json_resp.get('deprecated')}"
    except requests.RequestException as e:
        assert False, f"Request failed: {e}"

test_post_api_webhook_polar_deprecated()