...
def test_TC021_post_invoices_generate_pdf_no_body():
    ...
    try:
        response = requests.post(url, json={}, cookies=cookies, headers=headers, timeout=30)
    except requests.RequestException as e:
        assert False, f"Request failed: {e}"

    assert response.status_code == 400, f"Expected status code 400 but got {response.status_code}. Response text: {response.text}"

test_TC021_post_invoices_generate_pdf_no_body()