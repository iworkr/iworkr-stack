...
def test_tc055_get_care_plan_reviews_preview_missing_plan_id():
    cookies = get_auth_cookies()
    url = f'{BASE_ENDPOINT}/care/plan-reviews/preview'
    try:
        # Make GET request without the required 'planId' query param but with valid session cookies
        response = requests.get(url, cookies=cookies, timeout=30)
        # Expecting HTTP 400 Bad Request due to missing required planId query param
        assert response.status_code == 400, f"Expected 400, got {response.status_code} with body: {response.text}"
    except requests.RequestException as e:
        assert False, f"Request failed: {e}"

test_tc055_get_care_plan_reviews_preview_missing_plan_id()
