...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign in with Password' button to reveal the email and password input fields (index 2031).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div[6]/div/div[2]/button[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the email into the email field (index 2097), then type the password into the password field (index 2092), then click the 'Sign In' button (index 2099).
        frame = context.pages[-1]
        # Input text
        ...
        assert await frame.locator("xpath=//*[contains(., 'technician')]").nth(0).is_visible(), "Expected 'technician' to be visible"
