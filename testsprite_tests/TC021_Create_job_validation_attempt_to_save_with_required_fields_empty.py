...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign in with Password' button to open the email/password login form (click element index 2025).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div[6]/div/div[2]/button[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the test email into the email field (index 2094), then type the password into the password field (index 2089), then click the 'Sign In' button (index 2096).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Retry signing in once more (after a short wait). If login still fails (error persists and no /dashboard), report failure and finish the test.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div[6]/div/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
