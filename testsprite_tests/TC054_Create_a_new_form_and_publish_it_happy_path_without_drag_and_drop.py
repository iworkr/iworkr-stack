...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign in with Password' button to reveal email and password input fields.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div[6]/div/div[2]/button[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the test email into the email field (index 2093), type the test password into the password field (index 2088), then click 'Sign In' (index 2095).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Retry clicking the 'Sign In' button (index 2095) to attempt login again and reach the dashboard so the remaining form creation steps can be executed.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div[6]/div/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        assert await frame.locator("xpath=//*[contains(., 'Publish')]").nth(0).is_visible(), "Expected 'Publish' to be visible"
