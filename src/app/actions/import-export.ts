...
export async function importClientsFromCSV(
  ...
  try {
    ...
    console.error("[import-export] importClientsFromCSV failed:", e);
    return { imported: 0, skipped: 0, errors: [e?.message || "An unexpected error occurred"] };
  }
}
...
export async function exportWorkspaceData(): Promise<{
  ...
  try {
    ...
    console.error("[import-export] exportWorkspaceData failed:", e);
    return { clients: [], jobs: [], invoices: [], error: e?.message || "An unexpected error occurred" };
  }
}
