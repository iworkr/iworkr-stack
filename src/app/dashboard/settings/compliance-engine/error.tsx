/**
 * @error app/dashboard/settings/compliance-engine/error.tsx
 * @status STABLE
 * @description Error boundary — app/dashboard/settings/compliance-engine
...
/**
 * Compliance Engine (Solon-Law) Error Boundary
 *
 * Catches crashes from the RAG-based compliance engine settings
 * (framework management, ingestion pipeline, audit logs). Isolates
 * the failure so the rest of the dashboard remains functional.
 */
export default function ComplianceEngineError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[Compliance Engine Error Boundary]", {
    ...
    if (
      ...
      Sentry.captureException(error, {
        tags: { module: "compliance-engine", digest: error.digest },
      });
    }
  }, [error]);
    ...
    <div className="flex h-full items-center justify-center p-8">
      ...
      >
        ...
        <p className="mt-2 text-[13px] text-zinc-500">
          The compliance engine settings failed to load. Active compliance
          rules continue to operate in the background.
        </p>
