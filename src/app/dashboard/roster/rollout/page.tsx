...
const EASE_STEALTH: [number, number, number, number] = [0.25, 0.46, 0.45, 0.94];
...
const fadeIn = {
  ...
  visible: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: { delay: i * 0.03, duration: 0.35, ease: EASE_STEALTH },
  }),
};
...
export default function RolloutReviewPage() {
  ...
  const loadPreview = useCallback(async () => {
    ...
    try {
      ...
      const result = await generateRolloutPreview(
        templateId,
        orgId,
        start,
        weeks,
  ...
  const handleCommit = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Commit failed unexpectedly",
      );
      setShowConfirmModal(false);
    } finally {
      setCommitting(false);
  ...
  const successfulProjections = useMemo(
    () =>
      preview?.projections.filter(
        (p) => p.status === "ok" || p.status === "flagged",
      ) || [],
    [preview],
  );

  // Conflicts are rendered from preview.conflicts directly
  ...
  const groupedSuccess = useMemo(
    () => groupByDate(successfulProjections),
  ...
  const unassignedCount = useMemo(
    () =>
      preview?.projections.filter(
        (p) =>
          p.status === "conflict" &&
          p.conflict?.resolution?.type === "unassigned",
      ).length || 0,
    [preview],
  );
  ...
  if (!templateId && !loading) {
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6">
        ...
        >
          <div className="border border-zinc-800/50 rounded-xl bg-[#0A0A0A] p-8 text-center">
            <div className="w-14 h-14 rounded-full bg-zinc-800/50 flex items-center justify-center mx-auto mb-5">
            ...
            <p className="text-sm text-zinc-500 mb-6 leading-relaxed">
              ...
              The engine will check leave, compliance, budgets, and public
              holidays before creating shifts.
            </p>
            <button
  ...
  if (loading || orgLoading) {
    return (
      <div className="min-h-screen bg-[#050505] p-6">
        {/* Header skeleton */}
        <div className="mb-8">
          <div className="h-4 w-32 bg-zinc-800/60 rounded animate-pulse mb-3" />
          <div className="h-8 w-72 bg-zinc-800/60 rounded animate-pulse mb-2" />
        ...
        <div className="grid grid-cols-5 gap-3 mb-8">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="border border-zinc-800/50 rounded-xl bg-[#0A0A0A] p-4"
            >
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6">
        <motion.div
          ...
          className="max-w-md w-full border border-red-900/50 rounded-xl bg-[#0A0A0A] p-8 text-center"
        >
          <div className="w-14 h-14 rounded-full bg-red-500/10 flex items-center justify-center mx-auto mb-5">
            <X className="w-7 h-7 text-red-400" />
          </div>
          <h2 className="text-lg font-semibold text-zinc-100 mb-2">
            Rollout Preview Failed
          </h2>
          <p className="text-sm text-red-400/80 mb-6">{error}</p>
          <div className="flex items-center justify-center gap-3">
            <button
              onClick={() => router.push("/dashboard/roster/master")}
              className="px-4 py-2 rounded-lg border border-zinc-800 text-zinc-400 text-sm hover:bg-zinc-900 transition-colors"
            >
              Back to Master
            </button>
            <button
  ...
  return (
    <div className="min-h-screen bg-[#050505] relative">
      ...
      <AnimatePresence>
        {committed && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-[#050505]/90 backdrop-blur-sm flex items-center justify-center"
          >
            <motion.div
            ...
            >
              ...
              <p className="text-sm text-zinc-500">
                {committableCount} shifts pushed to live roster. Redirecting to schedule…
              </p>
            </motion.div>
          </motion.div>
        )}
        ...
        {showConfirmModal && (
          ...
          >
            <motion.div
              ...
              className="max-w-lg w-full border border-zinc-800 rounded-xl bg-[#0A0A0A] p-6"
            >
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-400 leading-relaxed">
                    ...
                    . {preview.summary.skipped > 0 && (
                      <>
                        {preview.summary.skipped} shifts will be skipped (holiday
                        cancellations).
                      </>
                    )}
                  </p>
              ...
              <div className="grid grid-cols-3 gap-2 mb-6">
                <div className="rounded-lg bg-emerald-500/[0.06] border border-emerald-500/20 p-3 text-center">
                  ...
                  <div className="text-[11px] text-emerald-500/70 uppercase tracking-wider mt-0.5">
                    Clean
                  </div>
                </div>
                <div className="rounded-lg bg-amber-500/[0.06] border border-amber-500/20 p-3 text-center">
                  ...
                  <div className="text-[11px] text-amber-500/70 uppercase tracking-wider mt-0.5">
                    Flagged
                  </div>
                </div>
                <div className="rounded-lg bg-red-500/[0.06] border border-red-500/20 p-3 text-center">
                  ...
                  <div className="text-[11px] text-red-500/70 uppercase tracking-wider mt-0.5">
                    Conflicts
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3">
                <button
                  ...
                  disabled={committing}
                  className="px-4 py-2 rounded-lg border border-zinc-800 text-zinc-400 text-sm hover:bg-zinc-900 transition-colors disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={committing}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-colors disabled:opacity-70"
                >
                  {committing ? (
                    <>
      ...
      {/* ── Page Content ─────────────────────────────────────── */}
      <div className="max-w-[1440px] mx-auto px-6 py-8 pb-28">
        {/* ── Header ───────────────────────────────────────────── */}
        <motion.header
        ...
        >
          <button
            onClick={() => router.push("/dashboard/roster/master")}
            className="inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-300 transition-colors mb-4 group"
          >
          ...
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-3xl font-semibold text-zinc-100 tracking-tight mb-1.5">
                Rollout Preview
              </h1>
            ...
            <button
              ...
              disabled={committed}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-600/20"
            >
        ...
        <div className="grid grid-cols-12 gap-4">
          {/* ── Left: Successful Projections (col-span-7) ──────── */}
          <div className="col-span-7">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-zinc-600 font-mono ml-auto">
                {successfulProjections.length} shifts
              </span>
            </div>

            <div className="space-y-4">
              {Array.from(groupedSuccess.entries()).map(
                ([date, projections], groupIdx) => {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {projections.some((p) => p.is_public_holiday) && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-400 text-[10px] font-medium uppercase tracking-wider">
              ...
              {successfulProjections.length === 0 && (
                <div className="border border-zinc-800/50 rounded-xl bg-[#0A0A0A] p-8 text-center">
                  <p className="text-sm text-zinc-500">
                    No successful projections. All shifts have conflicts.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="mt-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs text-zinc-700 font-mono ml-auto">
                    {skippedProjections.length} shifts
                  </span>
                </div>
                <div className="space-y-1.5">
                  {skippedProjections.map((proj, i) => (
                    <motion.div
                      ...
                      initial="hidden"
                      animate="visible"
                      className="flex items-center gap-3 px-4 py-2.5 rounded-lg border border-zinc-800/30 bg-zinc-900/30 opacity-60"
                    >
                      <Ban className="w-4 h-4 text-zinc-600 shrink-0" />
                      <span className="text-xs text-zinc-500 font-mono">
                        {formatDate(proj.target_date).formatted}
                      </span>
                      <span className="text-xs text-zinc-500 font-mono">
                        {formatTime(proj.start_datetime)} –{" "}
                        {formatTime(proj.end_datetime)}
                      </span>
                      <span className="text-xs text-zinc-600 line-through">
                        {proj.title || proj.support_purpose || "Care Shift"}
                      </span>
                      <span className="ml-auto px-2 py-0.5 rounded-full bg-zinc-800/50 text-zinc-600 text-[10px] font-medium uppercase">
                        Cancelled
                      </span>
                    </motion.div>
                  ))}
          ...
          {/* ── Right: Conflicts & Warnings (col-span-5) ───────── */}
          <div className="col-span-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-zinc-600 font-mono ml-auto">
                {preview.conflicts.length} issues
              </span>
            </div>
            ...
            {preview.conflicts.length > 0 ? (
              ...
              >
                ...
                <p className="text-xs text-zinc-600 mt-1">
                  All shifts passed leave, compliance, budget, and holiday
                  checks.
                </p>
              </motion.div>
            )}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-zinc-800/50 bg-[#050505]/95 backdrop-blur-lg">
          ...
          <div className="flex items-center gap-6">
            ...
            {preview.summary.conflicts > 0 && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-zinc-500">
                  conflict{preview.summary.conflicts !== 1 ? "s" : ""}
                </span>
              </div>
            )}
            ...
            {unassignedCount > 0 && (
              <p className="text-xs text-amber-500/80 max-w-xs">
                {unassignedCount} conflict{unassignedCount !== 1 ? "s" : ""} will
                create unassigned shifts requiring immediate cover
              </p>
            )}
            ...
            <button
              onClick={() => setShowConfirmModal(true)}
              disabled={committed}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-colors disabled:opacity-50 shadow-lg shadow-blue-600/20"
            >
              <Check className="w-4 h-4" />
...
function StatCard({
  ...
  const styles: Record<string, { pill: string; number: string; border: string }> = {
    ...
    zinc: {
      pill: "bg-zinc-700/30 text-zinc-500",
  ...
  return (
    <div
      className={`border ${s.border} rounded-xl bg-[#0A0A0A] p-4 flex flex-col justify-between`}
    >
      <div className="flex items-center justify-between mb-3">
        ...
        {icon && (
          <span
            className={`inline-flex items-center justify-center w-5 h-5 rounded-full ${s.pill}`}
          >
            {icon}
          </span>
        )}
      </div>
      <span className={`text-2xl font-semibold font-mono ${s.number} tracking-tight`}>
        {value}
      </span>
    </div>
  );
...
function ShiftCard({ projection }: { projection: RolloutProjection }) {
  ...
  return (
    <motion.div
      ...
      initial="rest"
      whileHover="hover"
      className={`flex items-center gap-4 px-4 py-3 rounded-lg border ${borderStyle} ${borderColor} bg-[#0A0A0A] transition-colors group`}
    >
      {/* Time range */}
      <div className="shrink-0">
        <span className="font-mono text-sm text-zinc-300">
          {formatTime(projection.start_datetime)}
        </span>
        <span className="text-zinc-600 mx-1">–</span>
        <span className="font-mono text-sm text-zinc-300">
          {formatTime(projection.end_datetime)}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        >
          {projection.assigned_worker_id
            ? getInitials(projection.assigned_worker_name)
            : "?"}
        </div>
        <span className="text-sm text-zinc-300 truncate">
          {projection.assigned_worker_name || (
            <span className="text-red-400 italic">Unassigned</span>
          )}
        </span>
      </div>

      {/* Title / support purpose */}
      <span className="text-xs text-zinc-600 truncate hidden lg:block">
        {projection.title || projection.support_purpose || ""}
      </span>

      {/* NDIS code */}
      {projection.ndis_line_item && (
        <span className="ml-auto shrink-0 px-2 py-0.5 rounded bg-zinc-800/60 text-[10px] font-mono text-zinc-500 uppercase">
          {projection.ndis_line_item}
        </span>
      )}
      ...
      {projection.is_public_holiday && (
        <span className="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-400 text-[10px] font-medium">
          <Flag className="w-2.5 h-2.5" />
          PH
        </span>
      )}
      ...
      {/* Status indicator */}
      {projection.status === "flagged" && (
        <AlertTriangle className="w-3.5 h-3.5 text-amber-400 shrink-0" />
      )}
      {projection.status === "ok" && (
        <Check className="w-3.5 h-3.5 text-emerald-500/60 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
      )}
    </motion.div>
  );
}
...
function ConflictCard({
  ...
  return (
    <motion.div
      ...
      animate="visible"
      className={`border ${
        isHardBlock ? "border-red-500/25" : "border-amber-500/20"
      } rounded-xl bg-[#0A0A0A] p-4 transition-colors hover:bg-[#0F0F0F]`}
    >
      {/* Header row */}
      <div className="flex items-start justify-between gap-3 mb-2.5">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span
              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider ${style.bg} ${style.text}`}
            >
              {style.label}
            </span>
          </div>
          ...
          {conflict.resolution.type === "backup_worker" && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-emerald-400">
                Backup: {conflict.resolution.backup_worker_name || "Assigned"}
              </span>
          ...
          {conflict.resolution.type === "unassigned" && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-red-400">
                Requires manual assignment — shift will be created unassigned
              </span>
            </div>
          )}
          {conflict.resolution.type === "skip" && (
