/**
 * @page app/dashboard/governance/policies/loading.tsx
 * @status STABLE
 * @description Loading UI — app/dashboard/governance/policies
    ...
    <div className="flex h-full flex-col bg-[#050505]">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="h-9 w-36 rounded-full bg-white/[0.04] animate-pulse"
              />
            ))}
      ...
      <div className="flex-1 space-y-3 overflow-hidden px-6 py-4">
        {Array.from({ length: 7 }).map((_, i) => (
          ...
          >
            <div className="h-10 w-9 shrink-0 rounded-md bg-white/[0.06] animate-pulse" />
            <div className="min-w-0 flex-1 space-y-2">
              <div className="h-3.5 max-w-lg rounded bg-white/[0.06] animate-pulse" />
              <div className="flex flex-wrap gap-2">
            ...
            <div className="h-5 w-24 shrink-0 rounded-full bg-white/[0.04] animate-pulse" />
          </div>
        ))}
      </div>
    </div>
