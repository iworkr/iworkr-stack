...
/**
 * Aegis-Zero: Store-and-Forward Webhook Pattern
 *
 * Phase 1 — STORE: Verify signature, insert into inbound_webhooks_queue, return 200 instantly.
 * Phase 2 — FORWARD: Process the event from the queue (inline, with DLQ fallback on failure).
 *
 * The inline processing is attempted immediately. If it fails, the row is left in
 * the queue with 'failed_retrying' status and exponential backoff for the pg_cron
 * worker to pick up. This gives us the best of both worlds: low latency on the
 * happy path, and zero data loss on the failure path.
 */
export async function POST(req: NextRequest) {
  ...
  // ── PHASE 1: STORE ── Insert into the queue for guaranteed delivery
  const { data: queued, error: queueErr } = await supabase
    ...
    .insert({
      ...
      headers: { stripe_event_id: event.id, api_version: event.api_version },
      status: "processing",
    })
    .select("id")
    .single();
  ...
  if (queueErr) {
    ...
    console.error("[Stripe Webhook] Queue insert failed:", queueErr.message);
    return NextResponse.json({ error: "Queue insert failed" }, { status: 500 });
  }

  const webhookId = queued.id;

  // ── PHASE 2: FORWARD ── Attempt inline processing (non-blocking for Stripe)
  // If this fails, the row stays in queue for the pg_cron worker
  try {
    await processStripeEvent(event, supabase);

    await supabase.rpc("resolve_webhook", { webhook_id: webhookId });
  } catch (err: any) {
    console.error(`[Stripe Webhook] Inline processing failed for ${event.type}:`, err.message);
