...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='fleet_positions')
    ...
    $fn$;
  ELSE
    RAISE NOTICE '[044] Skipping get_live_dispatch — fleet_positions or jobs table not found.';
  END IF;
END $$;

-- ── 2. update_fleet_position RPC ──
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='fleet_positions') THEN
    EXECUTE $fn$
      ...
      BEGIN
        ...
        -- Only insert into position_history if it exists
        BEGIN
    ...
    $fn$;
  ELSE
    RAISE NOTICE '[044] Skipping update_fleet_position — fleet_positions table not found.';
  END IF;
END $$;
