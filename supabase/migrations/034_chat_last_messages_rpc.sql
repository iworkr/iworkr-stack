...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'messages')
     AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'channel_members')
  THEN
    EXECUTE $fn$
      ...
      $body$;
    $fn$;
    RAISE NOTICE '[034] Created get_last_messages_for_channels function.';
  ELSE
    RAISE NOTICE '[034] Skipping — messages/channel_members tables do not exist yet.';
  END IF;
END $$;
